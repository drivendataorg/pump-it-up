...
d1$status_group_id[d1$status_group == 'non functional'] = 1;
d1$status_group_id[d1$status_group == 'functional needs repair'] = 1;

d1$status_group_bin[d1$status_group == 'functional'] = 'to repare';
d1$status_group_bin[d1$status_group == 'non functional'] = 'no repare';
d1$status_group_bin[d1$status_group == 'functional needs repair'] = 'no repare';



# remove duplicate variables
d1$status_group = NULL
d2$status_group = NULL
...
features <- c('subvillage', 'funder', 'installer','ward','lga')
#head(d1[, features], 20)
#head(d2[, features], 20)
...
tab = xgb.cv(data = dtrain, watchlist = watch_list, objective = "binary:logistic", booster = "gbtree",
                 nrounds = 10000, nfold = 10, early.stop.round = 10, 
                 maximize = FALSE,
                 evaluation = "logloss", eta = 0.05, 
...
valid.out$status_group_bin[valid.out$status_group_id > 0.25] = 'no repare';
valid.out$status_group_bin[valid.out$status_group_id <= 0.25] = 'to repare';
confusionMatrix(as.data.frame(valid.out)$status_group_bin, as.data.frame(d1valid)$status_group_bin)
